...
// --- Floaty Navigation Toggle ---
let navCooldown = false;

window.toggleNav = function() {
    const nav = document.querySelector('.floaty-nav');
    const launcher = document.querySelector('.nav-launcher-btn');
    if (nav) {
...
window.showNav = function() {
    ...
    // Set a brief cooldown to prevent touch/click events from immediately triggering scroll hiding
    navCooldown = true;
    setTimeout(() => {
        navCooldown = false;
    }, 400);
...
// Auto-hide navigation bar on scroll (slides to Menu, only moves back on touch/click)
window.addEventListener('scroll', (e) => {
    if (navCooldown) return;
    
    // Get scroll position from nested wrapper or window
    const wrapper = document.querySelector('.page-wrapper') || document.querySelector('.container') || document.documentElement;
    const scrollTop = wrapper.scrollTop || window.scrollY || document.documentElement.scrollTop;

    // Only hide if scrolled past threshold
    if (scrollTop > 50) {
        const nav = document.querySelector('.floaty-nav');
        const launcher = document.querySelector('.nav-launcher-btn');
        if (nav && !nav.classList.contains('hidden-nav')) {
            nav.classList.add('hidden-nav');
            if (launcher) {
                launcher.style.display = 'flex';
            }
...
}, true); // Capture scroll events from any nested scrollable container like .page-wrapper
