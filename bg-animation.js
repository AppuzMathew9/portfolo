...
// --- Page sync: show canvas only on home, hide on other pages ---
function syncCanvasVisibility() {
    ...
    if (isHomePage()) {
        ...
        animRunning = false; // pauses the rAF loop
    }
}
...
window.addEventListener('popstate', syncCanvasVisibility);
document.addEventListener('pagechange', syncCanvasVisibility);
// Polling fallback for SPA router
setInterval(syncCanvasVisibility, 400);
