...
function isHomePage() {
    ...
    return url.includes('index.html') || url.endsWith('/') || (!url.includes('.html') && !url.includes('#'));
}
...
// Dynamically toggle on SPA navigation
function syncShapesVisibility() {
    container.style.display = isHomePage() ? 'none' : 'block';
}
window.addEventListener('popstate', syncShapesVisibility);
document.addEventListener('pagechange', syncShapesVisibility);
// Also poll briefly after transitions complete
setInterval(syncShapesVisibility, 400);
